...
- **最優先**: VRChat実機で説明・設定・デバッグの3パネルのボタンが反応することを確認する。2026-08-18にビーム面をCanvasルートのImage/Buttonへ作り替えた。もし反応しない場合の次手は、`UiBeamTarget` 側へUdonBehaviourを移し、1つのColliderにUI面とInteract面を兼ねさせること。
- 複数人インスタンスで入退室の通知音とトーストを確認する。約3秒の受付窓で1グループ1回に集約され、行が `ほか○名` へ更新されることも対象。自分の入室時に一斉通知が鳴らないこと、音と表示を別々にOFFできること、5言語の文言、VRでの頭部追従の快適性が対象。2026-08-16にラジオ音量を絶対ローカル音量へ変更し、Slider Canvasへtrigger Collider、SceneへEventSystemを追加した。構造・数値・再生成はPass済みで、残るのは実音声と実操作の証拠だけ。
