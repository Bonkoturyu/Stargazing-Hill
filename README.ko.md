...
현재 시각과 모두가 공유하는 22개 관측 지점 중 하나에 맞춰 실제 별과 달을 보여 주는 VRChat용 조용한 초원 월드입니다.

## 주요 특징

- HYG v4.1의 실제 항성 데이터를 UTC와 전역으로 선택한 22개 지점의 위도·동경에 따라 회전
- 현재 달의 겉보기 위치 표시
...
- 나무 옆에서 들 수 있는 로컬 설정 보드: 5방향 거울 개별 ON/OFF와 공통 LQ/HQ, 나이트 모드 슬라이더, 날짜·시간, 알람, 라디오 스피커 ON/OFF·음량, 입퇴장 알림음·표시(각각 전환), 선택 저장
- 별하늘의 북쪽을 로컬로 가리키는 이동 가능한 CC0 나침반
...
환경, 실제 별하늘, 대기 소산, 전역 공유 22개 지점 선택기, 달 계산, 유성 시스템, YamaPlayer 표준 재생 목록 작업 흐름, 드로잉 도구, 5개 언어 패널, 인원 및 입퇴장 기록, 디버그 조작, 로컬 편의 설정과 휴대용 로컬 북쪽 나침반을 구현했습니다. 설정 보드와 나침반은 Unity 컴파일 및 저장된 Scene 구조 검증을 통과했으며, Windows／Android／iOS 실제 기기 최종 검증은 아직 진행 중입니다.
