...
- 显示当前实例人数和本地进出记录
- 树旁可手持的本地设置板：五方向镜面各自ON/OFF与共用LQ/HQ、夜间模式滑杆、日期时间、闹钟、收音机扬声器ON/OFF与音量、进出提示音与显示（可分别开关）、自选保存
- 可移动的CC0指南针；指针会在本地端指向星空的北方
...
环境、真实星空、大气消散、全局共享的22地点选择器、月亮计算、流星系统、YamaPlayer 标准播放列表流程、绘图工具、五语面板、人数和进出记录、调试控制、本地舒适度设置及手持本地北向指南针均已实现。设置板和指南针已通过 Unity 编译及保存场景结构验证；Windows／Android／iOS 真机最终验证仍未完成。
