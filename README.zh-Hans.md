...
这是一个为 VRChat 制作的宁静草原世界。星空和月亮会根据当前时间与全体共享的20个观测地点之一显示。

## 主要特色

- 使用 HYG v4.1 的真实恒星数据，并根据 UTC 与全局选择的20个地点之纬度、东经旋转天球
- 显示月亮当前的视位置
...
环境、真实星空、大气消散、全局共享的20地点选择器、月亮计算、流星系统、YamaPlayer 标准播放列表流程、绘图工具、五语面板、人数和进出记录、调试控制及移动平台 Shader 均已实现。ClientSim 多人测试以及 Windows／Android／iOS 真机最终验证仍未完成。
