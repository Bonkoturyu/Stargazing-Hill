...
- 顯示目前實例人數與本機進出紀錄
- 樹旁可手持的本機設定板：五方向鏡面各自ON/OFF與共用LQ/HQ、夜間模式滑桿、日期時間、鬧鐘、收音機喇叭ON/OFF與音量、進出提示音與顯示（可分別開關）、自選保存
- 可移動的CC0指南針；指針會在本機端指向星空的北方
...
環境、真實星空、大氣消散、全域共享的22地點選擇器、月亮計算、流星系統、YamaPlayer 標準播放清單流程、繪圖工具、五語面板、人數與進出紀錄、除錯控制、本機舒適度設定及手持本機北向指南針均已實作。設定板與指南針已通過 Unity 編譯及儲存場景結構驗證；Windows／Android／iOS 實機最終驗證尚未完成。
...
推送 `v*` 標籤後，GitHub Actions 會建立並檢查可再散布的 UnityPackage，再將它與 SHA-256 校驗檔一起封裝成 ZIP 並附加到對應的 Release。此流程不需要 Unity Editor 或 Unity 授權，也不包含 YamaPlayer、QvPen 與付費的 UnyStylus 檔案。Actions runner已確認可用；第一次以標籤建立Release仍為 `Pending Evidence`。請參閱[公開稽核（日文）](docs/PUBLIC_RELEASE_AUDIT.md)。
