...
一本木のそばに、初期状態では非表示のローカル設定ボードを置く。木に面一で取り付けた小型の歯車ボタンから各ユーザーが個別に表示する。ボタンは利用者がScene上で確定した `Position (8.261, 2.331, 7.755)`、`Rotation Quaternion (0.627459, 0.332323, -0.356062, 0.607517)`、`Scale 0.46967` を生成正本とする。文字フォントに依存しない単一メッシュの歯車形状を使い、見た目は小さいまま、操作面だけをworld約0.30m角へ逆スケール補正する。操作表示は `設定を開く / Open local settings` とする。ボードは利用者がScene上で確定した `Position (8.515, 2.778, 7.217)` / `Y Rotation 59.832°` を生成正本とし、約0.62 × 0.57mの手持ちサイズにする。ボタンとボードの位置は独立した確定値として保持する。上端の細いグリップだけをPickup領域にすることで、板面のUI操作と持ち運びを分離する。proximityは1.2mとし、板へ密着しなくても持てるようにする。`VRCObjectSync`は付けず、ボードのTransformとドロップ10秒後の初期位置復帰もローカルに処理する。時計・状態を上段、ミラーと暗さを左列、アラーム・ラジオ・保存を右列へまとめる。各見出しの左余白へ単色アイコンを置く。アイコンは第三者素材ではなく、木の歯車と同じくコードで生成した平面メッシュに `Unlit/Color` を当てたものとし、ミラー＝ベッド、ナイトモード＝月、アラーム＝目覚まし時計、ラジオ音量＝スピーカー、入退室通知＝出入口を割り当てる。生成方式にすることで、権利記録の追加も、テクスチャの読み込みも、線の太さの不揃いも発生しない。初期表示は日本語とし、右上の言語ボタンで日本語 / English / 繁體中文 / 简体中文 / 한국어をローカルに循環する。
...
- ミラー配置: 4面は敷物の実メッシュから測った外周に沿って立て、辺から0.06m外側で角が突き合うよう外周いっぱいの幅を持たせる。接地高さは外周4隅の最低地点ひとつに統一し、上り側は地面へ埋める。面ごとに接地させると角に隙間が空き、斜面で段差になったためである。高さは2.85m。敷物の保存Transformは実際のマット中心からずれるため、配置基準にしない。天井面は敷物中心の2.55m上に敷物の平面寸法で置く
