...
## 2026-08-20 追記7: 取っ手を側面へ、ボタンを角丸へ

- **入退室表示が自分のマイクアイコンと重なっていた。** 「ほか○名」が付く長い行で下端が伸びるためで、視線から0.30m下では足りなかった。0.16m下へ上げる。あわせて文字を0.062から0.048へ落とす。1.15mの距離で0.062は大きすぎ、3行出ると視界のかなりを塞いでいた。
- **アラームのアイコンが見出しの罫線を跨いでいた。** アイコンは共通の正方形から描いているが、埋め方は図ごとに違う。目覚まし時計は上のベルが他のどの図より高く伸びる。y 0.745から0.705へ下げた。目視でしか気付けなかったので、Scene検証に `ValidateIconClearsDividers` を足し、アイコンの実メッシュ範囲が `HeaderDivider` `ColumnDivider` と重なったら落とすようにした。
- **取っ手を上端1本から左右2本へ。** 板の縁から外へ出る位置に細い棒を1本ずつ立て、掴み判定もその外側だけに置く。**盤面に一切かからない**ので、判定を大きく取ってもボタンのレイを奪わない。上端に置いていたときは、判定を広げるほど言語ボタンと干渉する板挟みだった。
  - VRChatの操作レイは近い順に判定し、最初に見つけた対象を返す。掴み判定はビーム的にはボタンより手前（z方向で前）に来るため、XYで少しでも重なるとボタンより先に拾ってしまう。盤面から完全に外すのが唯一の確実な解になる。
  - 判定が2つになったので、`WorldSettingsBoardPickup` と `WorldDebugPanelPickup` に `secondPickupCollider` を足し、復帰中の有効・無効を `SetGripsEnabled` で揃える。
