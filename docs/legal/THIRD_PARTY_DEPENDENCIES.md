...
| UnyStylus | 1.3 | 描画 | 購入済みUnityPackage。VN3利用条件に従いワールドへ組込み、packageと展開本体はGitへ再配布しない。シーン参照の復元には購入済みv1.3のローカルimportが必要。Android / iOS GLES3のfog変数再定義だけを `Tools/Apply-UnyStylusPatches.ps1` で版・SHA-256限定修正する | Confirmed |
| YamaPlayer (`net.kwxxw.yama-stream`) | 2.0.0-beta.7 | 動画・音声再生 | 公式VPM `https://vpm.kwxxw.net/index.json` / 公式GitHub `koorimizuw/YamaPlayer`。READMEの利用条件に従い、VRChatワールドへの改変・組込み可。取得日 2026-08-11、package ZIP SHA-256 `fcf94198526e63319ebde966a479f4be7179c58d22f7e54493d9e75034ae2e9d`。VPM依存として解決しpackage本体は追跡しない。Editor自動更新確認とruntime Playlist build再利用の局所パッチは [ADR 0012](../adr/0012-yamaplayer-editor-update-check-patch.md) に限定する | Confirmed |
